...
def get_similarity_matrix(dataset_gradient, pb_gradient, dataset_train_length, pb_train_length):
    ...
    embedding_dim = dataset_gradient.shape[1]
    assert dataset_gradient.shape[1] == pb_gradient.shape[1], \
        f"Mismatch in representation dimensions: {dataset_gradient.shape[1]} vs {pb_gradient.shape[1]}"

    # This part for representation
    A = dataset_gradient.reshape((dataset_train_length, embedding_dim))
    B = pb_gradient.reshape((pb_train_length, embedding_dim))
