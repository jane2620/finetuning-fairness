...
import csv
import os
from prompt_extensions import *

def format_prompt(user_message):
    return f"<|user|>\n{user_message}\n<|assistant|>"


def format_prompt_no_bias(user_message):
    return f"<|user|>\n{user_message} {illegal_extension}\n<|assistant|>"
...
def collect_responses(prompts, model, tokenizer, BASE_MODEL, FT_DATASET, seed, num_samples=5, batch_size=16):
    ...
    prompts["formatted_prompt"] = prompts["prompt_text"].apply(format_prompt_no_bias)
    ...
    output_path = f"results/{FT_DATASET}/prompt_illegal/{BASE_MODEL.split('/')[-1]}_salinas_expanded_context_{seed}.csv"
